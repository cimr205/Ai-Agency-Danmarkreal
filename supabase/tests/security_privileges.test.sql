...
select ok(position('get_user_company_id(auth.uid()) = _company_id' in pg_get_functiondef('public.set_company_mode(uuid,text)'::regprocedure)) > 0, 'company mode mutation binds requested tenant to caller');
select ok(position('get_user_company_id(auth.uid()) = _company_id' in pg_get_functiondef('public.update_compliance_item(uuid,text,boolean)'::regprocedure)) > 0, 'compliance mutation binds requested tenant to caller');
