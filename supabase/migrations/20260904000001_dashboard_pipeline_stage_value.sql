-- Dashboard redesign: pipeline section now shows value and stalled-deal risk
-- per stage (not just a headcount), so the frontend can render real insight
-- instead of a bare pie chart. Adds two fields to each pipeline stage row;
-- everything else in get_dashboard_summary() is unchanged from the previous
-- migration.
...
as $function$
  ...
  -- Value and "stalled" (open, untouched 10+ days — same threshold focus_deals
  -- uses below) per stage, so the dashboard can show more than a headcount.
  ...
  select jsonb_build_object(
    ...
    'pipeline', jsonb_build_object(
      'stages', coalesce(
        (select jsonb_agg(jsonb_build_object('name', name, 'color', color, 'count', count, 'value', value, 'stalled', stalled)) from pipeline_stage_rows),
        '[]'::jsonb
      )
    ),
