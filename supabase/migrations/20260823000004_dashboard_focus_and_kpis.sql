-- Dashboard redesign: replace the "recent activity" debug log with a real
-- "Dagens fokus" action list (overdue invoices, stalled deals, cold leads),
-- and give the KPI row numbers that actually matter to an agency owner
-- (invoiced this month + MoM trend, overdue amount, open pipeline value,
-- new leads this month) instead of raw all-time totals with no comparison.
