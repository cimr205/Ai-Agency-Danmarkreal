...
export function PhoneDeviceConnection({ platform, connectedPhone, onConnectionChange }: PhoneDeviceConnectionProps) {
  ...
  if (connectedPhone) {
    ...
    return (
      <Card className="border-border border-l-2 border-l-emerald-500 bg-card p-4 shadow-none">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center border border-emerald-500/30 bg-emerald-500/10 text-emerald-600"><CheckCircle2 className="h-4 w-4" /></div>
            <div>
    ...
    return (
      <Card className="overflow-hidden border-border bg-card shadow-none">
        <div className="border-b border-border p-4">
          <div className="flex items-start gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center border border-border bg-background text-primary"><Laptop className="h-4 w-4" /></div>
            <div>
          ...
          <div>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {(Object.keys(SETUPS) as RelaySetup[]).map((setup) => (
                 <button key={setup} type="button" aria-pressed={relaySetup === setup} onClick={() => { setRelaySetup(setup); setCompletedSteps([]); }} className={cn('border px-3 py-2.5 text-left text-xs font-medium transition-colors', relaySetup === setup ? 'border-foreground bg-foreground text-background' : 'border-border bg-background hover:border-foreground/40')}>
          ...
          {relaySetup === 'mac_android' ? (
            <div className="border border-amber-500/30 bg-amber-500/5 p-4">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
                <div className="mx-auto shrink-0 border border-border bg-white p-2 sm:mx-0" aria-label={t('devicePowerDialer.relay.macAndroid.qrLabel')}>
                  ...
                  <ol className="mt-4 space-y-2">
                    {[1, 2, 3, 4].map((step) => (
                      <li key={step} className="flex gap-2 text-sm">
                         <span className="flex h-5 w-5 shrink-0 items-center justify-center border border-amber-500/30 bg-amber-500/10 font-mono text-[10px] font-semibold text-amber-700">{step}</span>
          ...
          <div className="border border-border bg-background p-4">
            ...
            <div className="space-y-3">
              {steps.map((step) => {
                ...
                return (
                   <label key={step} className={cn('flex cursor-pointer items-start gap-3 border p-3 transition-colors', checked ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-border hover:bg-muted/30')}>
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <a href={config.helpUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline">{t('devicePowerDialer.relay.officialHelp')}<ExternalLink className="h-3 w-3" /></a>
               <Button disabled={!allStepsComplete} className="gap-2 rounded-none bg-foreground text-background shadow-none hover:bg-foreground/90 hover:shadow-none" onClick={() => onConnectionChange(connectComputerRelay(config.computer, config.phone))}>
  ...
  return (
    <Card className="border-border border-l-2 border-l-primary bg-card p-4 shadow-none">
      <div className="flex items-start gap-3">
         <div className="flex h-10 w-10 shrink-0 items-center justify-center border border-border bg-background text-primary"><Smartphone className="h-4 w-4" /></div>
        <div className="min-w-0 flex-1 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
            <div className="space-y-2"><Label htmlFor="power-dialer-own-number">{t('devicePowerDialer.pairing.ownNumberLabel')}</Label><Input id="power-dialer-own-number" type="tel" inputMode="tel" autoComplete="tel" value={phoneNumber} onChange={(event) => { setPhoneNumber(event.target.value); setValidationError(false); }} placeholder={t('devicePowerDialer.pairing.ownNumberPlaceholder')} aria-invalid={validationError} />{validationError ? <p className="text-xs text-destructive">{t('devicePowerDialer.pairing.invalidNumber')}</p> : null}</div>
             <Button onClick={handleConnect} className="gap-2 rounded-none bg-foreground text-background shadow-none hover:bg-foreground/90 hover:shadow-none"><Link2 className="h-4 w-4" />{t('devicePowerDialer.pairing.connectAction')}</Button>
          </div>
